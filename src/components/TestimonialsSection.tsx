...
const testimonials = [
  {
    id: 1,
    quote: "Digital Mudrana delivered outstanding quality for our annual reports. The colors were vibrant, the paper was premium, and they met our tight deadline without a hitch.",
    name: "Sarah Johnson",
  ...
  {
    id: 3,
    quote: "From concept to completion, Digital Mudrana handled our event materials with professionalism and precision. We'll definitely be using their services for all future events.",
...
const TestimonialsSection = () => {
  ...
  return (
    <section id="testimonials" ref={sectionRef} className="py-20 bg-gradient-to-b from-print-darkPurple to-print-background">
      <div className="container-section">
        <div ref={headingRef} className="text-center mb-16">
          <div className="flex justify-center mb-4">
            <Users className="h-10 w-10 text-print-gold" />
          </div>
          <h2 className="text-3xl md:text-4xl font-bold text-print-gold mb-4">What Our Clients Say</h2>
        ...
        <div ref={testimonialsRef} className="grid grid-cols-1 md:grid-cols-2 gap-8">
          {testimonials.map((testimonial) => (
            <div key={testimonial.id} className="testimonial-card">
              <p className="mb-6 text-print-text italic">{testimonial.quote}</p>
              <div className="flex items-center">
                ...
                <div>
                  <h4 className="font-semibold text-print-gold">{testimonial.name}</h4>
                  <p className="text-sm text-print-peach">{testimonial.title}</p>
                </div>
              </div>
            </div>
          ))}
